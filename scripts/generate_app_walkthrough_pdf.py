#!/usr/bin/env python3
"""Generate a detailed professor-facing application walkthrough PDF."""
...
def footer(canvas, doc):
    ...
    canvas.drawString(
        0.75 * inch,
        0.35 * inch,
        "UML Multimodal Studio — Application Walkthrough (Dipak Yadav)",
    )
...
def build():
    ...
    story.append(Paragraph("UML Multimodal Studio", s["title"]))
    story.append(
        Paragraph(
            "Complete Application Walkthrough — How Every Page Works",
            s["subtitle"],
        )
    )
    story.append(
        Paragraph(
            "Thesis: <i>Automated UML Dataset Generation from Natural-Language Requirements "
    ...
    story.append(
        Paragraph(
            "This is an end-to-end <b>thesis demonstration system</b>. It turns a plain-English "
    ...
    story.append(
        Paragraph(
            ...
            "persistence. By default the system runs in <b>mock provider mode</b> so a professor "
            "demo needs no paid API keys. Real OpenAI-compatible or Ollama models can be enabled "
    ...
    story.append(
        Paragraph(
            ...
            "same orchestration path. This section is what you should explain as the scientific core.",
            s["body"],
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            "It confirms the API is reachable and gives a thesis-oriented orientation.",
            s["body"],
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            "an offline panel and stops. Use this page to prove the stack is healthy before a demo.",
            s["body"],
        )
    )
    ...
    story.append(
        Paragraph(
            "Purpose: a project health overview for thesis evaluation — counts, score health, and recent work.",
            s["body"],
        )
    )
    ...
    # 5. Single Generation
    story.append(Paragraph("5. Single Generation (main demo page)", s["h1"]))
    story.append(
        Paragraph(
            "Purpose: the primary interactive demo. One requirement (or code snippet) → one or more diagrams "
    ...
    story.append(
        Paragraph(
            "Purpose: build a larger demo dataset quickly for thesis statistics (e.g., 50 requirements × 5 types).",
            s["body"],
        )
    )
    story.append(Paragraph("Step-by-step use", s["h2"]))
    ...
    story.append(
        Paragraph(
            "Purpose: browse and inspect any stored artifact in depth — the “evidence room” for the thesis.",
            s["body"],
        )
    )
    story.append(Paragraph("Step-by-step use", s["h2"]))
    ...
    story.append(
        Paragraph(
            "Purpose: quantitative summary for the thesis results chapter / demo slides.",
            s["body"],
        )
    )
    story.append(Paragraph("Step-by-step use", s["h2"]))
    ...
    story.append(
        Paragraph(
            "Purpose: verify runtime configuration before or during a professor demo.",
            s["body"],
        )
    )
    story.append(Paragraph("Step-by-step use", s["h2"]))
    story.append(bullets([
        ...
        "If Java is missing, note that remote PlantUML rendering is active (still OK for demos).",
        "If mock providers are ON, explain this is intentional for offline thesis demos.",
    ], s))
    ...
    story.append(
        table(
            [
                ...
                [Paragraph("MOCK_PROVIDERS", s["small"]), Paragraph("true = offline demo without paid APIs (default)", s["small"])],
    ...
    story.append(
        table(
            [
                ...
                [Paragraph("Single Generation", s["small"]), Paragraph("Interactive paper pipeline demo", s["small"]), Paragraph("Yes — creates artifacts", s["small"])],
                [Paragraph("Batch Generation", s["small"]), Paragraph("Scale demo / dataset building", s["small"]), Paragraph("Yes — many artifacts", s["small"])],
    ...
    story.append(
        Paragraph(
            ...
            "flowchart/activity rows help reach the 8,000 total. This supports the thesis dataset story "
    ...
    # 13. Demo script
    story.append(Paragraph("13. Recommended 5-minute professor demo script", s["h1"]))
    story.append(bullets([
        "<b>0:30</b> — Home: show API connected, mock provider, thesis title.",
    ...
    # 14. Honesty
    story.append(Paragraph("14. Honest scope notes (say this if asked)", s["h1"]))
    story.append(bullets([
        ...
        "Default mock mode produces content-aware but simulated LLM/VLM behavior for reliable offline demos.",
