...
Local copy on the original Mac: `/Users/dslc/Desktop/UML`
...
2. Retrain PlantUML LoRA with **PyTorch + PEFT + CUDA** (new path; repo only has `scripts/finetune_plantuml.py` for **MLX**). Point `USE_FINETUNED_CODE` at a CUDA provider **or** serve a fine-tuned HF model via vLLM/Ollama.
...
```
   ...
   (Use port **8001** if FastAPI already owns **8000**.)
...
**NVIDIA:** implement CUDA LoRA from the same `data/finetune/*.jsonl` (Qwen2.5-0.5B or larger). Do not call `mlx_lm`.
...
USE_FINETUNED_CODE=true  # Apple MLX only unless you add CUDA
USE_AYA=true
VLM_AYA_BACKEND=openai_compat
AYA_VLM_BASE_URL=http://127.0.0.1:8001/v1
...
Parser: `uml_pipeline/llm_client.py` `parse_score_response` must accept `**SEMANTIC: 5**` and not treat `<0-6>` as score 0.
