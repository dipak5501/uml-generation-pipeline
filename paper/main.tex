...
inter-component relationships~\cite{booch2005},\cite{ambler2004}. In practice,
...
revised~\cite{shinde2012},\cite{maatuk2021},\cite{abdelnabi2020}.

Recent advances in large language models (LLMs) have created new
opportunities for automating requirement-driven software
modeling~\cite{vaswani2017},\cite{brown2020}. Compared with earlier rule-based
...
structured artifacts with greater flexibility~\cite{meng2024},\cite{yuan2025}.
...
% Bottom row: verification and output
\node[box, below=1.2cm of uml] (vlm)   {VLM Ensemble\\Qwen / LLaMA-V / Aya};
\node[box, below=0.8cm of vlm] (score) {MMMU-Weighted\\Score + Majority Vote};
\node[box, left=1.2cm of score](ds)    {Validated UML\\Dataset};
...
% render feeds down into VLM ensemble
\draw[line] (render.south) |- (vlm.east);
...
% Feedback arrows: req and spec feed into VLM from below
\draw[line] (req.south)  -- ++(0,-0.55) -| (vlm.north west);
\draw[line] (spec.south) -- ++(0,-0.35) -| ([xshift=0.2cm]vlm.north);
...
isolated at stage boundaries~\cite{wei2022},\cite{brown2020}.
...
tasks~\cite{jahan2024},\cite{bates2025}. Second, its compact parameter
...
across model invocations~\cite{wei2022},\cite{brown2020}.
